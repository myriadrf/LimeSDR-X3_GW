...
-- ----------------------------------------------------------------------------   
   ...
   port map(
      ...
      -- SPI 0 
      spi_0_MISO                 => inst0_spi_0_MISO OR inst6_sdout OR inst12_sdout or inst12_gnss_sdout,
...
--   end process;
   ...
   FPGA_LED4_G <= inst12_fpga_led_g;
   FPGA_LED4_R <= inst12_fpga_led_r;
...
-- ----------------------------------------------------------------------------
 ...
 -- RX and TX module
   ...
   port map(
      ...
      sclk              => inst0_spi_0_SCLK,    -- Data clock
      sen               => inst0_spi_0_SS_n(c_SPI0_FPGA_SS_NR), -- Enable signal (active low)
      sdout             => inst12_gnss_sdout,        -- Data out 
      -- Signals coming from the pins or top level serial interface
      lreset            => reset_n,    -- Logic reset signal, resets logic cells only  (use only one reset)
      mreset            => reset_n,    -- Memory reset signal, resets configuration memory only (use only one reset)
      vctcxo_clk        => LMK1_CLK1,    -- Clock from VCTCXO       
      --LimeGNSS-GPIO pins
      gnss_tx           => open,   
...
--	port map (a_in => inst0_gpo(0), a_out => FPGA_ADC_RESET);
   ...
   -- LMS2 tx ant en
   PMOD_A_PIN8 <= inst12_tx_ant_en when inst0_from_periphcfg.PERIPH_OUTPUT_OVRD_0(8)='0' else inst0_from_periphcfg.PERIPH_OUTPUT_VAL_0(8);  
   -- LMS1 tx ant en
   PMOD_A_PIN9 <= inst6_tx_ant_en when inst0_from_periphcfg.PERIPH_OUTPUT_OVRD_0(9)='0' else inst0_from_periphcfg.PERIPH_OUTPUT_VAL_0(9);
